...
class Neusoft:
    ...
    #policy snat RenHangGongJiaoICKa netmask 10.52.0.0 255.252.0.0 iplist 9.80.41.56 napt enable
        ...
        ip1 = ''
        ip2 = ''
    #policy snat qz_49_001_src iplist 10.9.9.49 iplist 192.168.207.49 napt enable
    #policy snat new_czh_50_002_src iplist 10.9.9.50 iplist 172.16.25.6 napt enable
    ...
    #下面是目的地址
        ...
        elif 'holdtime' in words:
            mode = 'dynamicip'
            m = words.index('holdtime')
            trans_to_ip = words[m-1]+'/32'
        status = ''
        eif = ''
        if 'disable' in words:
    ...
    def neusoft_dnat(self,alist,j):
#policy dnat DG_CHANGANJIUDIAN_BMP 192.168.207.18 10.9.9.51 enable
 #policy dnat DG_CHANGANJIUDIAN_BMP matching input-interface eth5
 #policy dnat DG_CHANGANJIUDIAN_BMP matching sip 8.0.4.138
 #policy dnat DG_CHANGANJIUDIAN_BMP matching sip 192.168.12.1
 #policy dnat DG_CHANGANJIUDIAN_BMP matching sip 172.12.12.2 172.12.12.3
 #policy dnat DG_CHANGANJIUDIAN_BMP matching sip 192.168.42.1
 #policy dnat DG_CHANGANJIUDIAN_BMP matching sip 8.0.4.189
        dnat = ''
        service = ''
        priv_port = ''
        words= alist[j].split()
        m= words.index('dnat')
        descrip = words[m+1]
        srcaddrname = 'dnat_' + descrip + '_srcip'
#policy dnat policy_name before_trans_ipaddress [domain domain_name] {after_trans_ipaddress | {tcp | udp | other} before_trans_port after_trans_ipaddress after_trans_port} {enable | disable} [pri]
        if 'tcp' in words or 'udp' in words or 'other' in words:
            if 'tcp' in words:
                n = words.index('tcp')
                proto = 'tcp'
            elif 'udp' in words:
        ...
        while (j < len(alist)):
            ...
            if descrip not in words:
                break
            if 'input-interface' in words:
                m = words.index('input-interface')
                inif = words[m+1]

            if 'sip' in words:#目的地址，
                #policy snat RenHangGongJiaoICKa matching dip netmask 9.80.40.114 255.255.255.255
                if 'netmask' in words:
                    m = words.index('netmask')
                    type = 'ip'
                    sip = words[m+1] + '/' + asa_exchangemask(words[m+2])
                else:
                    m = words.index('sip')
                    if len(words)>(m+2):
                        type = 'range'
                        sip = words[m+1] + ' ' + words[m+2]
                    else:
                        type = 'ip'
                        sip = words[m + 1]+'/32'
                query = "insert into addrbok_in_dnat(addrname,type,ip1) values(?,?,?);"
                tmplist = [srcaddrname, type, sip]
                insert2table_new(query, tmplist)
            # if 'append' in words and 'before' in words:#源地址
            #     if 'ip' in words:
            #         m = words.index('ip')
            ...
            #         insert2table_new(query, tmplist)
            j += 1
        query = "insert into dnat(inif,srcaddr,pub_ip,service,trans_to,port,status,description) values(?,?,?,?,?,?,?,?);"
        tmplist = [inif,srcaddrname, pubip, service,  privip, priv_port, status,descrip]
        insert2table_new(query, tmplist)
        j -= 1
    ...
    def neusoft_policy(self,alist,j):
        policy = ''
        words = alist[j].split()
        try:
    ...
    def neusoft_build_policy_fromdb(self):
        policy = ''
    ...
    def neusoft_build_addrbok_fromdb(self):
        addrbok = ''
        addrbok_ip = ''
    ...
    def neusoft_build_service_fromdb(self):
        service = ''
        service_text = ''
    ...
    def neusoft_fetch_onesnataddrbok_fromdb(self,addrname):
        addrbok = ''
        addrbok_ip =''
    ...
    def neusoft_build_snat_fromdb(self):
        snatrule = ''
    ...
    def is_neusoft(self,fileuri):
        ...
        qurey = "CREATE TABLE IF NOT EXISTS addrbok_in_dnat(id integer primary key autoincrement, \
                                addrname TEXT, \
                                type TEXT, \
                                ip1 TEXT, \
                                ip2 TEXT);"
        ...
        while (i < len(srcfile)):
            ...
            try:
                if 'policy' in words and 'snat'  in words:
                    i = Neusoft.neusoft_snat(self,srcfile,i)
                if 'policy access ' in srcfile[i]:
                    i = Neusoft.neusoft_policy(self,srcfile, i)
                    pass
                if 'policy dnat ' in srcfile[i]:
                    i = Neusoft.neusoft_dnat(self,srcfile, i)
                if 'policy mip ' in srcfile[i]:
                    i = Neusoft.neusoft_bnat(self,srcfile, i)
                i += 1
            except:
                echo = '请检查第'+str(i+1)+'行'
                self.status = echo
        ...
        addrbook = ''
        addrbook = Neusoft.neusoft_build_addrbok_fromdb(self)
        writefile(newfileuri, addrbook)

        service = ''
        service = Neusoft.neusoft_build_service_fromdb(self)
        writefile(newfileuri, service)

        snat = Neusoft.neusoft_build_snat_fromdb(self)
        writefile(newfileuri, snat)

        writefile(newfileuri,dnat)
        writefile(newfileuri,bnat)

        policyconfig = Neusoft.neusoft_build_policy_fromdb(self)
